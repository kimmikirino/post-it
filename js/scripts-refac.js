...
var notes = {
    ...
    save: function (id, newTitle, newText) {
...
const formEdit = function (form, id, type) {
    ...
    if (type) {
        ...
        formEdit += '<input type="text" name="title"  class="note__title" value="' + note.title + '" />' +
            '<textarea name="body" rows="5" class="note__body" >' + note.text + '</textarea>' +
            '<button class="note__control" type="button" onclick="updateNote(this.form, this.form.title, this.form.body, ' + note.id + ')">' +
            'Atualizar' +
            '</button>';
    } else {
        ...
        formEdit += '<h1 type="text" name="title"  class="note__title">' + note.title + '</h1>' +
            '<p name="body" rows="5" class="note__body">' + note.text + '</p>';
    }
